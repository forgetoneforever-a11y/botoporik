...
# Сначала создаем бота и диспетчер (обязательно вверху перед хендлерами!)
bot = Bot(token=TOKEN)
dp = Dispatcher()
...
# Список всех текстов кнопок меню, чтобы бот не путал их с приветствием
MENU_BUTTONS = [
...
# Приветствие (теперь dp уже существует)
